...
@inquiry_router.message(InquiryState.faq_question_id, F.text == "📁 Boʻlimlar")
...
@inquiry_router.message(InquiryState.faq_question_id, F.text == "⬅️ Asosiy menyu")
...
@inquiry_router.message(InquiryState.faq_question_id, F.text == "✅ Ha")
...
@inquiry_router.message(InquiryState.faq_question_id, F.text == "❌ Yoʻq")
@inquiry_router.message(InquiryState.faq_question_id, F.text == "📩 Savol yuborish")
...
async def select_section(message: Message, state: FSMContext, db_session: DBSession):
    ...
    markup.button(text="📩 Savol yuborish")
    markup.button(text="📁 Boʻlimlar")
    markup.button(text="⬅️ Asosiy menyu")

    answer_message = "\n\n".join([f"{i + 1}. {faq['q']}" for i, faq in enumerate(faqs[:40])])
    await message.answer(f"""
{answer_message}\n\n
⬆️ Tepada keltirilgan savollardan biriga javob olish uchun savol raqamini kiriting.\n
Agar boshqacha savol bermoqchi bo’lsangiz, o’z savolingizni yuborish uchun "📩 Savol yuborish" tugmasini bosing.
""",
